...
MouthEncoder::MouthEncoder()
    : n_("~")
{
  mouth_shape_sub_ = n_.subscribe<std_msgs::UInt8>("mouth_shape", 100, &MouthEncoder::mouthShapeCallback, this);
  encoded_mouth_pub_ = n_.advertise<std_msgs::UInt8MultiArray>("/idmind_interaction/mouth", 100);

  ros::param::param<int>("~number_of_mouths", number_of_mouths_, 5);
...
void MouthEncoder::readMouths()
{
  ...
  std::ifstream my_file((homedir + std::string("/catkin_ws/src/no_build/config/behaviours/mouths.txt")).c_str());
