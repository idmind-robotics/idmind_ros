    ...
    : n_("~"), zeroes_(0), latched_counter_(0), autonomous_(true), latched_(false), publishing_(false), reconnecting_(false)
{
  ...
  state_sub_ = n_.subscribe<std_msgs::UInt8MultiArray>("/state", 100, &IdmindTeleoperation::stateCallback, this);
...
void IdmindTeleoperation::runPeriodically()
{
  ...
  while(n_.ok())
  {
    ...
    if (!autonomous_ && ros::Time::now() > (last_received_ + ros::Duration(0.015))) // --> 12.5ms TODO turtlebot_teleop_key @ 30Hz, not enough
    {
...
void IdmindTeleoperation::stateCallback(const std_msgs::UInt8MultiArray::ConstPtr& msg)
{
  autonomous_ = (*msg).data[0];
}
